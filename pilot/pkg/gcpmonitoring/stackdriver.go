...
// TeeToStackdriver configures a parallel logging pipeline that writes logs to the Google Cloud Logging API.
func TeeToStackdriver(project, logName string, mr *monitoredres.MonitoredResource) log.Extension {
	return TeeToStackdriverWithQuotaProject(project, project, logName, mr)
}

// TeeToStackdriverWithQuotaProject configures a parallel logging pipeline that writes logs to the Google Cloud Logging API.
func TeeToStackdriverWithQuotaProject(project, quotaProject, logName string, mr *monitoredres.MonitoredResource) log.Extension {
	return func(c zapcore.Core) (zapcore.Core, func() error, error) {
		return teeToStackdriver(c, project, quotaProject, logName, mr)
	}
}
