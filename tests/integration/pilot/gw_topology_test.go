...
func TestUpstreamProxyProtocol(t *testing.T) {
	framework.
		...
		Run(func(t framework.TestContext) {
			...
			// we only apply to config clusters
			// make custom gateway in same ns as app, so we can apply the destination rule
			// otherwise, the destinationrule will be ignorred by the `restrict-to-namespace` sidecar
			gatewayNs := apps.Namespace
			t.ConfigIstio().Eval(gatewayNs.Name(), templateParams, `apiVersion: v1
kind: Service
metadata:
  name: custom-gateway
