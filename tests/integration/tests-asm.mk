...
.PHONY: test.integration.asm.cloudesf.apikeyhttpbackend
test.integration.asm.cloudesf.apikeyhttpbackend: | $(JUNIT_REPORT)
	PATH=${PATH}:${ISTIO_OUT} $(GO) test -p 1 ${T} -tags=integ $(shell go list -tags=integ ./tests/integration/cloudesf/apikeyhttpbackend | grep -v "${DISABLED_PACKAGES}") -timeout 30m \
	${_INTEGRATION_TEST_FLAGS} ${_INTEGRATION_TEST_SELECT_FLAGS} --log_output_level=tf:debug \
	2>&1 | tee >($(JUNIT_REPORT) > $(JUNIT_OUT))
...
# Custom test target for GSM networking minimal
.PHONY: test.integration.gsm.mcp.networking.minimal
test.integration.gsm.mcp.networking.minimal: | $(JUNIT_REPORT)
	PATH=${PATH}:${ISTIO_OUT} $(GO) test -p 1 ${T} -tags=integ $(shell go list -tags=integ ./tests/integration/pilot/... | grep -v "${DISABLED_PACKAGES}") -run TestTraffic/gateway -timeout 60m \
	${_INTEGRATION_TEST_FLAGS} ${_INTEGRATION_TEST_SELECT_FLAGS} --log_output_level=tf:debug \
	2>&1 | tee >($(JUNIT_REPORT) > $(JUNIT_OUT))
...
test.integration.asm.longrunning: | $(JUNIT_REPORT)
	PATH=${PATH}:${ISTIO_OUT} $(GO) test -p 1 ${T} -tags=integ $(shell go list -tags=integ ./tests/integration/longrunning/... | grep -v "${DISABLED_PACKAGES}") -timeout 480m \
	${_INTEGRATION_TEST_FLAGS} ${_INTEGRATION_TEST_SELECT_FLAGS} --log_output_level=tf:debug \
	2>&1 | tee >($(JUNIT_REPORT) > $(JUNIT_OUT))
