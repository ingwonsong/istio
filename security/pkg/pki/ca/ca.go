...
func NewSelfSignedIstioCAOptions(ctx context.Context,
	...
	// TODO(jaellio): caCertName is initialized to "istio-ca-secret" here, but it is overwritten as "cacerts"
	// if the secret is not found. This is confusing and will be cleaned up once "istio-ca-secret" to "cacerts"
	// migration is done.
	// Initially checks for the existence of "istio-ca-secret" and if it does not exist, attempts to create
	// "cacerts". If creation fails, it will retry the check for the existence of "cacerts" and creating the
	// secret until it succeeds or the time limit is reached.
	caCertName := CASecret
	...
	err = b.RetryWithContext(ctx, func() error {
		...
		if err == nil {
			// TODO(jaellio): If the secret is not generated by istio and named "cacerts", ignore it.
		...
		if apierror.IsNotFound(err) {
			...
			secret := BuildSecret(caCertName, namespace, nil, nil, nil, pemCert, pemKey, istioIntermediateCASecretType)
